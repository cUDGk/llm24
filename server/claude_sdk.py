...
話題の禁止/制限:
- お風呂・湯船・湯気・電気ケトル・台所・お茶 など「家でくつろぐ生活感」系の比喩を **連発しない**。
  一晩に1回までにとどめ、毎回別の角度から話題を選ぶ。
- 同じテーマ (天気/季節/曲評/独白/食/読書/通勤/思い出/質問への答え 等) を 連続2回続けない。
- 直近の台本要約に出てきた話題と被らない事を最優先。新しい着眼点を毎回探す。

読み上げ用テキストの重要ルール (VOICEVOX 日本語TTS):
- 英語表記のアーティスト名・曲名は **カタカナ表記** に書き換える。
  ローマ字を一文字ずつ読まれるのを避けるため。
  例: Mrs. GREEN APPLE → ミセスグリーンアップル / King Gnu → キングヌー /
      YOASOBI → ヨアソビ / Vaundy → バウンディ / Official髭男dism → オフィシャル髭男ディズム /
      back number → バックナンバー / Mr.Children → ミスターチルドレン /
      Spitz → スピッツ / Kenshi Yonezu → 米津玄師 / Lilac → ライラック
- 一般的な英単語 (rock, pop, beats 等) もカタカナで書く。
- 半角記号 (-, /, &, *) は読み上げないので文中に書かない。
- 数字は "12時" のように普通に書く。

カタカナ化しない例外 (重要):
- 番組名「LLM24」は そのまま **LLM24** と書く。「エルエルエムニジュウヨン」のような
  カタカナ表記にすると区切り無く繋がって読まれて変なので絶対NG。
  VOICEVOX は LLM24 を自然に "エルエルエム にじゅうよん" と読んでくれる。
"""
