...
読み上げ用テキストの重要ルール:
- 英語のアーティスト名・曲名・一般単語は **原文の英語表記のまま** 書く。
  ブラウザ側で英語連続部分を自動検出し、英語TTS (Web Speech API) で
  ネイティブ発音される。カタカナへの書き換えはしない。
  例 OK: "次の曲は Mrs. GREEN APPLE の Lilac です"
  例 NG: "ミセスグリーンアップル" のようにカタカナ化
- ただし VOICEVOX で読まれる日本語パートが自然になるよう、英語と日本語の間に
  自然な助詞や句読点を入れる (例: "Mrs. GREEN APPLE の" のように "の" や ", ")。
- 半角記号 (/, &, * など) は読み上げに不要なので避ける。ハイフン入りタイトルは
  そのままでよい。
- 数字は "12時" のように普通に書く。
"""
